...
const Data = {
  ...
  loadProviders: function loadProviders (_, doneCallback) {
    ...
    loadDir(App.srcPath(PROVIDERS_DIR))
      .then(results => {
  ...
  loadModels: function loadModels (_, doneCallback) {
    ...
    loadDir(App.srcPath(MODELS_DIR))
      .then(results => {
  ...
  initModel: function initModel (model, config) {
    ...
    if (connection && !connection.disabled) {
      ...
      const initedModel = model.module.init(provider)

      if (model.module.multi) {
        for (const modelName in initedModel) {
          App.data.models[capitalize(modelName)] = initedModel[modelName]
        }
      } else {
        App.data.models[capitalize(model.name)] = initedModel
      }
